...
const Popup: React.FC<PopupProps> = ({
    ...
    const fetchBillReport = async () => {
        if (!authToken) {
            setBillError("Authentication token missing.");
            return;
        }
        setIsBillLoading(true);
        setBillError(null);
        const billEndpoint = `${apiBaseUrl}/api/dean/proposals/${selectedProposal.id}/bill`;
        try {
            console.log('Popup: Fetching bill report:', billEndpoint);
            const response = await axios.get<{ status: string; items: Item[] }>(billEndpoint, {
                headers: { Authorization: `Bearer ${authToken}`, Accept: 'application/json' }
            });
            console.log('Popup: Bill report fetched:', response.data);
            setBillItems(response.data.items);
            setShowBill(true);
        } catch (error: any) {
            console.error('Popup: Detailed error fetching bill report:', error);
            const axiosError = error as AxiosError;
            console.log('Popup: Bill error response:', axiosError.response);
            setBillError((axiosError.response?.data as any)?.message || axiosError.message || 'Failed to fetch bill report');
        } finally {
    ...
    const handleApprove = async () => {
        ...
        const approveEndpoint = `${apiBaseUrl}/api/dean/proposals/${selectedProposal.id}/approve`;
        if (!window.confirm('Are you sure you want to approve this proposal?')) return;
        try {
            setIsLoading(true); setErrorMessage(null);
            console.log('Popup: Approving proposal:', approveEndpoint);
            const response = await axios.post(
                approveEndpoint,
                {},
                { headers: { Authorization: `Bearer ${authToken}`, Accept: 'application/json', 'Content-Type': 'application/json' } }
            );
            console.log('Popup: Proposal approved:', response.data);
            onProposalUpdated(); closePopup();
        } catch (error: any) {
            console.error('Popup: Detailed error approving proposal:', error);
            const axiosError = error as AxiosError;
            console.log('Popup: Approve error response:', axiosError.response);
    ...
    const handleRequestClarification = async () => {
        ...
        const clarifyEndpoint = `${apiBaseUrl}/api/dean/proposals/${selectedProposal.id}/clarify`;
        try {
            setIsLoading(true); setErrorMessage(null);
            console.log('Popup: Requesting clarification:', clarifyEndpoint);
            const response = await axios.post(
                clarifyEndpoint,
                { message: clarificationInput },
                { headers: { Authorization: `Bearer ${authToken}`, Accept: 'application/json', 'Content-Type': 'application/json' } }
            );
            console.log('Popup: Clarification requested:', response.data);
            onProposalUpdated(); closePopup();
        } catch (error: any) {
            console.error('Popup: Detailed error requesting clarification:', error);
            const axiosError = error as AxiosError;
            console.log('Popup: Clarify error response:', axiosError.response);
    ...
    const handleReject = async () => {
        if (!authToken) { setErrorMessage("Authentication token missing."); return; }
        const rejectEndpoint = `${apiBaseUrl}/api/dean/proposals/${selectedProposal.id}/reject`;
        if (!window.confirm('Are you sure you want to reject this proposal? This action might be irreversible.')) return;
        try {
            setIsLoading(true); setErrorMessage(null);
            console.log('Popup: Rejecting proposal:', rejectEndpoint);
            const response = await axios.post(
                rejectEndpoint,
                { message: clarificationInput || 'Rejected by Dean' },
                { headers: { Authorization: `Bearer ${authToken}`, Accept: 'application/json', 'Content-Type': 'application/json' } }
            );
            console.log('Popup: Proposal rejected:', response.data);
            onProposalUpdated(); closePopup();
        } catch (error: any) {
            console.error('Popup: Detailed error rejecting proposal:', error);
            const axiosError = error as AxiosError;
            console.log('Popup: Reject error response:', axiosError.response);
            ...
            >
                ...
                <div className="p-4 md:p-6 space-y-4 overflow-y-auto flex-grow">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                        ...
                        <div><p className="text-sm font-medium text-gray-500">Status:</p><p className={`text-sm font-medium ${selectedProposal.status === 'Approved' ? 'text-green-600' : selectedProposal.status === 'Pending' ? 'text-yellow-600' : selectedProposal.status === 'Rejected' ? 'text-red-600' : 'text-blue-600'}`}>{selectedProposal.status || 'N/A'}</p></div>
                ...
                <div className="p-4 md:p-6 border-t border-gray-200 bg-gray-50 rounded-b-lg">
                    ...
                    {selectedProposal.status === 'Pending' && selectedProposal.awaiting === 'dean' && !isRejecting && !isClarifying && !isApproving && (
