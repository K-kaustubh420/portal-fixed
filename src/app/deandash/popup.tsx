...
import { UnifiedProposal, Item } from './deandash';
...
const Popup: React.FC<PopupProps> = ({
  ...
  const userRole = 'dean'; // Hardcoded for Dean-specific component
  ...
  const getActionEndpoint = () => {
    ...
    return `${apiBaseUrl}/api/dean/proposals/${proposalId}`;
  };
  ...
  const handleApprove = async () => {
    ...
    if (!canPerformApiActions || !approveEndpoint) {
      setErrorMessage(authToken ? 'API base URL or Proposal ID missing.' : 'Auth token missing.');
      return;
    }
    ...
    try {
      ...
      const axiosError = error as AxiosError;
      const errorMessage =
        (axiosError.response?.data as any)?.message || axiosError.message || 'Failed to approve proposal';
      console.error('Popup (Dean): Error approving proposal:', {
        errorMessage,
        status: axiosError.response?.status,
        proposalId: safeProposal.id,
      });
  ...
  const handleRequestClarification = async () => {
    ...
    try {
      ...
      const axiosError = error as AxiosError;
      const errorMessage =
        (axiosError.response?.data as any)?.message || axiosError.message || 'Failed to request clarification';
      console.error('Popup (Dean): Error requesting clarification:', {
        errorMessage,
        status: axiosError.response?.status,
        proposalId: safeProposal.id,
      });
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      const axiosError = error as AxiosError;
      const errorMessage =
        (axiosError.response?.data as any)?.message || axiosError.message || 'Failed to reject proposal';
      console.error('Popup (Dean): Error rejecting proposal:', {
        errorMessage,
        status: axiosError.response?.status,
        proposalId: safeProposal.id,
      });
