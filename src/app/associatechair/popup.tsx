...
const Popup: React.FC<PopupProps> = ({
  ...
  const getActionEndpoint = () => {
    ...
    return `${apiBaseUrl}/api/vice/proposals/${proposalId}`;
  };

  const fetchBillReport = async () => {
    // Restrict bill report to vice_chair only
    // To allow userRole === 'vice' as well, change to: if (userRole !== 'vice_chair' && userRole !== 'vice')
    if (userRole !== 'vice_chair') {
      setBillError('Bill report is only available for Vice Chair.');
      return;
    }
    ...
    setBillError(null);
    const billEndpoint = `${apiBaseUrl}/api/vice/proposals/${safeProposal.id}/bill`;
    try {
      ...
      console.error('Popup (Vice Chair Bill): Error fetching bill report:', {
        errorMessage,
  ...
  const isAwaitingCurrentUser = !!userRole &&
    (userRole.toLowerCase() === 'vice' || userRole.toLowerCase() === 'vice_chair') &&
    safeProposal.awaiting?.toLowerCase() === 'vice_chair';

  // Debug logging
  useEffect(() => {
    console.log('Popup Debug:', {
      status: safeProposal.status,
      awaiting: safeProposal.awaiting,
      userRole,
      isAwaitingCurrentUser,
      canPerformApiActions,
      isRejecting,
      isClarifying,
      isApproving,
    });
  }, [safeProposal, userRole, isAwaitingCurrentUser, canPerformApiActions, isRejecting, isClarifying, isApproving]);
    ...
    >
      ...
      >
        ...
        <div className="p-4 md:p-6 border-t border-gray-200 bg-gray-50 rounded-b-lg sticky bottom-0 z-10">
          ...
          {safeProposal.status.toLowerCase() === 'pending' && isAwaitingCurrentUser && !isRejecting && !isClarifying && !isApproving && (
          ...
          {safeProposal.status.toLowerCase() === 'pending' && !isAwaitingCurrentUser && (
            <div className="alert alert-info text-xs p-2 mb-3 shadow-md">
              <span>This proposal is not awaiting action from your role ({userRole}).</span>
              <span>Currently awaiting: {formatAwaiting(safeProposal.awaiting)}</span>
            </div>
          )}
