...
import { UnifiedProposal, Item, Sponsor, Message } from './ViceDashboard'; // Update import to ViceChairDashboard
...
const Popup: React.FC<PopupProps> = ({
    ...
    const handleApprove = async () => {
        ...
        const approveEndpoint = `${apiBaseUrl}/api/vice/proposals/${selectedProposal.id}`;
        ...
        try {
            ...
            console.error("ViceChair Approve error:", error);
    ...
    const handleRequestClarification = async () => {
        ...
        const clarifyEndpoint = `${apiBaseUrl}/api/vice/proposals/${selectedProposal.id}`;
        try {
            setIsLoading(true); setErrorMessage(null);
            await axios.put(
                clarifyEndpoint,
            ...
            console.error("ViceChair Clarification request error:", error);
            const axiosError = error as AxiosError;
    ...
    const handleReject = async () => {
        if (!authToken) { setErrorMessage("Authentication token missing."); return; }
        const rejectEndpoint = `${apiBaseUrl}/api/vice/proposals/${selectedProposal.id}`;
        ...
        try {
            ...
            console.error("ViceChair Reject error:", error);
            const axiosError = error as AxiosError;
        ...
        >
            ...
            >
                ...
                <div className="p-4 md:p-6 border-t border-gray-200 bg-gray-50 rounded-b-lg">
                    ...
                    {selectedProposal.status === 'Pending' && selectedProposal.awaiting === 'vice_chair' && !isRejecting && !isClarifying && !isApproving && (
