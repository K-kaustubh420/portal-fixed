...
const Popup: React.FC<PopupProps> = ({ selectedProposal, closePopup, onProposalUpdated }) => {
    ...
    const handleAccept = async () => {
        try {
            setIsLoading(true);
            setErrorMessage(null);
            await axios.put(`${API_BASE_URL}/proposals/${selectedProposal.id}`, { status: 'Approved' });
            onProposalUpdated(); // Notify parent component
            closePopup();
        } catch (error: any) {
            setErrorMessage(error.message || 'Failed to accept proposal');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleReject = async () => {
        try {
            ...
            setIsLoading(true);
            setErrorMessage(null);
            await axios.put(`${API_BASE_URL}/proposals/${selectedProposal.id}`, {
                status: 'Rejected',
                rejectionMessage: rejectionInput,
            });
            onProposalUpdated(); // Notify parent component
            closePopup();
        } catch (error: any) {
            setErrorMessage(error.message || 'Failed to reject proposal');
        } finally {
            setIsLoading(false);
        }
    };
                    ...
                    <div className="mt-6">
                        ...
                        <div className="flex space-x-4 justify-end">
                            ...
                            <button
                                ...
                                className="px-6 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md transition duration-300"
                                disabled={isLoading || !rejectionInput.trim()}
                            >
                    ...
                    <div className="mt-6">
                        ...
                        <div className="flex space-x-4 justify-end">
                            ...
                            <button
                                ...
                                disabled={isLoading || !reviewInput.trim()}
                            >
                                {isLoading ? 'Reviewing...' : 'Confirm Review'}
                            </button>
                        </div>
                    </div>
                )}
