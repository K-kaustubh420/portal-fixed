...
import Popup from './popup'; // Assumes Popup is correctly updated
...
// ProposalListItem remains the same
...
// --- NEW: Nested interfaces for DetailedProposal ---
...
interface DetailedProposal {
    ...
    created_at: string;
    updated_at: string;
    user: DetailedProposalUser; // Submitter details
    chiefs: DetailedProposalChief[] | null;
    items: DetailedProposalItem[];
    sponsors: DetailedProposalSponsor[];
    messages: DetailedProposalMessage[]; // <-- Updated to use the detailed message structure
}
...
interface PopupProposal {
    ...
    // Pass the detailed arrays directly as Popup expects them
    detailedBudget: DetailedProposalItem[];
    sponsorshipDetailsRows: DetailedProposalSponsor[];
    ...
    // Status & Messages
    awaiting?: string | null;
    messages: DetailedProposalMessage[]; // <-- Pass the full, structured messages array
...
const ConvenerDashboard: React.FC = () => {
    // State variables (proposals, loading, error, selectedProposalDetail, etc.)
    ...
    // useEffect for authorization and initial fetch (remains the same)
    useEffect(() => {
        if (isAuthLoading) { setLoading(true); setIsAuthorized(false); return; }
        if (user && token) { setIsAuthorized(true); fetchProposals(token, user); }
        else { setIsAuthorized(false); setLoading(false); }
    }, [isAuthLoading, user, token]); // Removed fetchProposals from deps array

    // fetchProposals callback (remains the same)
    const fetchProposals = useCallback(async (authToken: string, currentUser: User) => {
         if (!currentUser) return;
         setLoading(true); setError(null);
         const proposalEndpoint = `${API_BASE_URL}/api/faculty/proposals`;
         try {
             console.log(`Fetching proposals from: ${proposalEndpoint} for role: ${currentUser.role}`);
             const response = await axios.get<{ data: ProposalListItem[] } | ProposalListItem[]>(proposalEndpoint, { headers: { Authorization: `Bearer ${authToken}`, Accept: 'application/json' }, });
             ...
             if (Array.isArray(proposalsData)) {
                 ...
                 const cleanedData = proposalsData.map(p => ({ ...p, status: p.status?.toLowerCase() || 'unknown', user: p.user || { id: -1, name: 'Unknown User', email: 'N/A' } }));
                 setProposals(cleanedData);
             } else { console.error("API Error: Proposals data is not an array.", response.data); setProposals([]); setError("Received invalid proposal data format from server."); }
         } catch (err: any) {
             console.error("Error fetching proposals:", err);
             if (axios.isAxiosError(err)) { const axiosError = err as AxiosError; if (axiosError.response?.status === 401) { setError("Authentication error. Please log in again."); logout(); } else if (axiosError.code === 'ERR_NETWORK') { setError("Network error. Could not reach server."); } else { setError(`Failed to fetch proposals: ${(axiosError.response?.data as any)?.message || axiosError.message}`); } }
             else { setError(err.message || 'An unknown error occurred while fetching proposals'); }
             setProposals([]);
         } finally { setLoading(false); }
     }, [logout]);

    // --- UPDATED fetchProposalDetail callback ---
    const fetchProposalDetail = useCallback(async (proposalId: number | string) => {
        if (!isAuthorized || !token || !user) { setError("Cannot fetch details: Not authorized."); return; }
        ...
        try {
            ...
            // Expect the response structure { proposal: DetailedProposal }
            const response = await axios.get<{ proposal: DetailedProposal }>(detailEndpoint, {
             ...
             const apiData = response.data.proposal; // Access the nested proposal object
             if (!apiData) { throw new Error("Proposal data not found in API response."); }
             ...
             // --- Map to PopupProposal structure ---
             const detailedDataForPopup: PopupProposal = {
                ...
                messages: apiData.messages || [], // <-- Pass the full messages array directly
             };

             console.log("Mapped data for Popup:", detailedDataForPopup);
            ...
            console.error("Error fetching or processing proposal detail:", err);
             ...
             if (axios.isAxiosError(err)) { const axiosError = err as AxiosError; if (axiosError.response?.status === 401) { setError("Authentication error fetching details. Please log in again."); logout(); } else if (axiosError.response?.status === 404) { setError("Proposal details not found."); } else { setError(`Failed to load details: ${(axiosError.response?.data as any)?.message || axiosError.message}`); } }
             else { setError(`An error occurred: ${err.message}`); }
        } finally { setIsPopupLoading(false); }
    }, [isAuthorized, token, user, logout]);

    // --- Popup Handling ---
    const closePopup = () => setSelectedProposalDetail(null);
    const handleProposalUpdated = () => {
        if (token && user) { fetchProposals(token, user); }
        closePopup();
    };
    ...
    if (error) { return ( <div className="flex flex-col items-center justify-center h-screen p-4"> <div className="alert alert-error shadow-lg max-w-md mb-4"> <div> <svg xmlns="http://www.w3.org/2000/svg" className="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> <span>Error: {error}</span> </div> </div> {isAuthorized && token && user && (<button onClick={() => fetchProposals(token, user)} className="btn btn-primary">Retry Fetching Proposals</button>)} {!isAuthorized && (<button onClick={() => router.push('/login')} className="btn btn-secondary">Go to Login</button>)} </div> ); }
     if (!isAuthorized) { return ( <div className="flex flex-col justify-center items-center h-screen"> <span className="text-warning text-lg">Access Denied or Session Expired.</span> <button onClick={() => router.push('/login')} className="btn btn-primary mt-4">Login</button> </div> ); }


    // --- Data Derivations ---
    const validProposals = Array.isArray(proposals) ? proposals : [];
    const recentPendingOrReview = validProposals .filter(p => ['pending', 'review'].includes(p.status)) .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()) .slice(0, 5);
    ...
    const handleListItemClick = (proposalItem: ProposalListItem) => {
        if (proposalItem && proposalItem.id) { fetchProposalDetail(proposalItem.id); }
        else { console.error("Cannot fetch details: Invalid proposal item data.", proposalItem); setError("Could not open proposal details due to missing data."); }
    };
